<?php
    ...
    $oDepartamento = DepartamentoPDO::buscarDepartamentoPorCodigo($_SESSION['codDepartamentoEnCurso']);//sacamos el departamento seleccionado
    ...
    if(isset($_REQUEST['aceptar'])){ //Si el usuario pulsa el boton de ACEPTAR
            ...
            $aErrores['dDepartamento']=validacionFormularios::comprobarAlfabetico($_REQUEST['descripcionDepartamento'], 255, 4, 1); //Hago la validacion de la descripcion
            $aErrores['vNegocio']=validacionFormularios::comprobarFloat($_REQUEST['volumenNegocio'], 255, 1, 1); //Hago la validacion del volumen de negocio
    ...
    //SI TODO HA IDO BIEN
    if($entradaOk){
        DepartamentoPDO::modificarDepartamento($_SESSION['codDepartamentoEnCurso'], $desDepartamento, $volNegocio); //Guardo en la sesion el departamento valido
